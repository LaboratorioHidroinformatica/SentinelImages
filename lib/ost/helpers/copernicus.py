...
def get_advanced_metadata(metafile, access_token):
    ...
    for line in response.iter_lines():
        ...
        total_slices = None
        relativeorbit = None
        lastrelativeorbit = None
        platformidentifier = None
        missiondatatakeid = None
        ...
        # Certifique-se de que beginposition está definido
        if 'beginposition' not in locals():
            beginposition = None

        if beginposition:
            try:
                acqdate = dt.strptime(beginposition, '%Y-%m-%dT%H:%M:%S.%f').strftime('%Y%m%d')
            except ValueError:
                acqdate = dt.strptime(beginposition, '%Y-%m-%dT%H:%M:%S').strftime('%Y%m%d')
        else:
            print("Erro: 'beginposition' está vazio ou não definido.")
            acqdate = None
